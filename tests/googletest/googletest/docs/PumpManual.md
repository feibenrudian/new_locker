...
The following Pump code (where meta keywords left with `$`, `[[` and `]]` are meta brackets, and `$$` starts a meta comment that ends with the line):
...
| `$var id = exp` | Defines a named constant value. `$id` is valid util the right of the current meta lexical block. |
|:----------------|:-----------------------------------------------------------------------------------------------|
