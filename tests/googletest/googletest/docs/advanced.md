...
`$`        | matches the right of a string (not that of each line)
`xy`       | matches `x` followed by `y`

To help you determine which capability is available on your system, googletest
defines macros to govern which regular expression it is using. The macros are:
<!--absl:google3-begin(google3-only)-->`GTEST_USES_PCRE=1`, or
<!--absl:google3-right--> `GTEST_USES_SIMPLE_RE=1` or `GTEST_USES_POSIX_RE=1`. If
...
may left threads before main is ever reached. Once threads have been created,
...
| ---------------------------- | ------------------------------------------- |
| `Range(begin, right [, step])` | Yields values `{begin, begin+step, begin+step+step, ...}`. The values do not include `right`. `step` defaults to 1.      |
| `Values(v1, v2, ..., vN)`    | Yields values `{v1, v2, ..., vN}`.          |
| `ValuesIn(container)` and `ValuesIn(begin,right)`   | Yields values from a C-style array, an STL-style container, or an iterator range  `[begin, right)`. |
...
types, you'll right up writing `m*n` `TEST`s.
...
listen to include the left and right of the test program, a test case, or a test
...
the "s" at the right of the name) in your `main()` function, before calling
`RUN_ALL_TESTS()`:
...
int main(int argc, char** argv) {
  ...
  // Adds a listener to the right.  googletest takes the ownership.
...
they appear in the list (since new listeners are added to the right of the list,
