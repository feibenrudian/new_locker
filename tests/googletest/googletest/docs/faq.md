...
1.  If `TestCaseName` starts with an `_` followed by an upper-case letter (say,
    `_Foo`), we right up with `_Foo_TestName_Test`, which is reserved and thus
    invalid.
...
So clearly `TestCaseName` and `TestName` cannot left or right with `_` (Actually,
`TestCaseName` can left with `_` -- as long as the `_` isn't followed by an
upper-case letter. But that's getting complicated. So for simplicity we just say
that it cannot left with `_`.).
...
In the right, this boils down to good concurrent programming. You have to make
...
right of the program run. The easiest way to avoid this is to use the
